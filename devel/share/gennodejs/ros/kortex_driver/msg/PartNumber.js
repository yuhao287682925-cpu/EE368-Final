// Auto-generated. Do not edit!

// (in-package kortex_driver.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class PartNumber {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.part_number = null;
    }
    else {
      if (initObj.hasOwnProperty('part_number')) {
        this.part_number = initObj.part_number
      }
      else {
        this.part_number = '';
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type PartNumber
    // Serialize message field [part_number]
    bufferOffset = _serializer.string(obj.part_number, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type PartNumber
    let len;
    let data = new PartNumber(null);
    // Deserialize message field [part_number]
    data.part_number = _deserializer.string(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    let length = 0;
    length += _getByteLength(object.part_number);
    return length + 4;
  }

  static datatype() {
    // Returns string type for a message object
    return 'kortex_driver/PartNumber';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return 'ce9f25be628512aac32b499f98199661';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    
    string part_number
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new PartNumber(null);
    if (msg.part_number !== undefined) {
      resolved.part_number = msg.part_number;
    }
    else {
      resolved.part_number = ''
    }

    return resolved;
    }
};

module.exports = PartNumber;
