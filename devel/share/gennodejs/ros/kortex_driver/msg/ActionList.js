...
class ActionList {
  ...
  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type ActionList
    // Serialize message field [action_list]
    // Serialize the length for message field [action_list]
    bufferOffset = _serializer.uint32(obj.action_list.length, buffer, bufferOffset);
    obj.action_list.forEach((val) => {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type ActionList
    let len;
    let data = new ActionList(null);
    // Deserialize message field [action_list]
    // Deserialize array length for message field [action_list]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.action_list = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.action_list[i] = Action.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    let length = 0;
    object.action_list.forEach((val) => {
  ...
  static Resolve(msg) {
    ...
    const resolved = new ActionList(null);
    if (msg.action_list !== undefined) {
      resolved.action_list = new Array(msg.action_list.length);
      for (let i = 0; i < resolved.action_list.length; ++i) {
        resolved.action_list[i] = Action.Resolve(msg.action_list[i]);
      }
    }
    else {
