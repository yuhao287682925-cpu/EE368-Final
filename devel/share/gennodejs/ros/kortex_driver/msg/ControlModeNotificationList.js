...
class ControlModeNotificationList {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.notifications.forEach((val) => {
      bufferOffset = Base_ControlModeNotification.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.notifications[i] = Base_ControlModeNotification.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.notifications.forEach((val) => {
      length += Base_ControlModeNotification.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.notifications !== undefined) {
      ...
      for (let i = 0; i < resolved.notifications.length; ++i) {
        resolved.notifications[i] = Base_ControlModeNotification.Resolve(msg.notifications[i]);
      }
    }
    else {
