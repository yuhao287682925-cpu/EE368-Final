...
struct Printer< ::kortex_driver::MapEvent_events_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::kortex_driver::MapEvent_events_<ContainerAllocator>& v)
  {
    if (false || !indent.empty())
      s << std::endl;
    s << indent << "safety_event: ";
    if (v.safety_event.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.safety_event.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::kortex_driver::SafetyEvent_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.safety_event[i]);
    }
    if (v.safety_event.empty() || false)
      s << "]";
    ...
    for (size_t i = 0; i < v.gpio_event.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::kortex_driver::GpioEvent_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.gpio_event[i]);
    }
    if (v.gpio_event.empty() || false)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "controller_event: ";
    if (v.controller_event.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.controller_event.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::kortex_driver::ControllerEvent_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.controller_event[i]);
    }
    if (v.controller_event.empty() || false)
      s << "]";
  }
