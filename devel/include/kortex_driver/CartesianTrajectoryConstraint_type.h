  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::kortex_driver::CartesianTrajectoryConstraint_type_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.speed.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::kortex_driver::CartesianSpeed_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.speed[i]);
    }
    if (v.speed.empty() || false)
      s << "]";
    ...
    for (size_t i = 0; i < v.duration.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint32_t>::stream(s, true ? std::string() : indent + "    ", v.duration[i]);
    }
    if (v.duration.empty() || true)
      s << "]";
  }
