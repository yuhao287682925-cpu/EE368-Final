...
struct Printer< ::kortex_driver::CalibrationParameter_value_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::kortex_driver::CalibrationParameter_value_<ContainerAllocator>& v)
  {
    if (false || !indent.empty())
      s << std::endl;
    s << indent << "signedIntValue: ";
    if (v.signedIntValue.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.signedIntValue.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint32_t>::stream(s, true ? std::string() : indent + "    ", v.signedIntValue[i]);
    }
    if (v.signedIntValue.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "unsignedIntValue: ";
    if (v.unsignedIntValue.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.unsignedIntValue.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint32_t>::stream(s, true ? std::string() : indent + "    ", v.unsignedIntValue[i]);
    }
    if (v.unsignedIntValue.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.floatValue.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint32_t>::stream(s, true ? std::string() : indent + "    ", v.floatValue[i]);
    }
    if (v.floatValue.empty() || true)
      s << "]";
  }
