...
struct Printer< ::kortex_driver::MapGroup_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::kortex_driver::MapGroup_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "children_map_group_handles: ";
    if (v.children_map_group_handles.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.children_map_group_handles.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::kortex_driver::MapGroupHandle_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.children_map_group_handles[i]);
    }
    if (v.children_map_group_handles.empty() || false)
      s << "]";
    if (true || !indent.empty())
