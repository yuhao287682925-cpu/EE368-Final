...
struct Printer< ::kortex_driver::ProtectionZone_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::kortex_driver::ProtectionZone_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "limitations: ";
    if (v.limitations.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.limitations.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::kortex_driver::CartesianLimitation_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.limitations[i]);
    }
    if (v.limitations.empty() || false)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "envelope_limitations: ";
    if (v.envelope_limitations.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.envelope_limitations.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::kortex_driver::CartesianLimitation_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.envelope_limitations[i]);
    }
    if (v.envelope_limitations.empty() || false)
      s << "]";
  }
