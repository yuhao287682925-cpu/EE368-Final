...
class AutoContactDrawer:
    ...
    def run_auto_touchdown(self):
        ...
        while not rospy.is_shutdown():
            ...
            # 起步前 0.5 秒 (20 个周期) 内屏蔽判定，避开加速瞬间的电机抖动
            if loop_cnt > 20:
    ...
    def execute_and_draw(self, csv_file):
        ...
        rospy.loginfo("🔄 正在基于实际物理接触点在线重生成轨迹 (整体上移 5mm 释压)...")
        aligned_waypoints = []
        
        # 将 15N 强压点向上抬起 5mm 作为最终理论画板平面，以获得轻柔笔触
        base_z_nominal = contact_pose.position.z + 0.005
        
        for wp in raw_waypoints:
            aligned_wp = {
                'x': contact_pose.position.x + (wp['x'] - u_ref_x),
                'y': contact_pose.position.y + (wp['y'] - u_ref_y),
                'z_nominal': base_z_nominal + (wp['z_nominal'] - u_ref_z),
                'nx': wp['nx'],
                'ny': wp['ny'],
                'nz': wp['nz'],
                'phase': wp['phase'],
        ...
        for i, wp in enumerate(aligned_waypoints):
            ...
            while not rospy.is_shutdown():
                ...
                if phase not in ['draw', 'touch_down']:
                    ...
                    if dist > 0.001:
                        ux = v_target_x / dist
                        uy = v_target_y / dist
                        # 计算机械臂在运动方向上施加的合力 (克服阻力的推力)
                        f_push = self.current_fx * ux + self.current_fy * uy
                        
                        # 阻力阈值设为 12.0N (由于包含机械臂自身动摩擦，此值较高)
                        # 当推力超过阈值时，说明笔尖卡到坑洼或受到阻碍
                        if f_push > 12.0:
            ...
            if phase in ['draw', 'touch_down'] and (i % 2 == 0 or phase == 'touch_down'):
                ...
                # 辅助力控：微量调整 z_offset
                if static_fz > 5.0:
                    self.z_offset += 0.0008  # 压力偏大，微抬 0.4mm，加快释压避免卡死
                elif static_fz < 2.5:
