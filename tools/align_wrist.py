...
class WristAligner:
    def __init__(self):
        ...
        rospy.loginfo("   >> 仅控制前两个角: Roll=0.0, Pitch=180.0 (Yaw放开自转)")
        rospy.loginfo("   >> 解锁自转约束，完全避免奇异点与旋转方向错乱问题")
    ...
    def run(self):
        ...
        rospy.loginfo("🔄 正在对齐前两个角 (Roll/Pitch) 至垂直...")
        
        while not rospy.is_shutdown():
            # 计算 Roll 和 Pitch 的偏差值，完全不考虑 Yaw (自转角)
            e_roll = wrap_angle(target_roll - self.roll)
            e_pitch = wrap_angle(target_pitch - self.pitch)
            
            error_norm = math.sqrt(e_roll**2 + e_pitch**2)
            ...
            # 仅控制前两个角速度，第三个角速度 (自转) 直接设为 0
            cmd.twist.angular_x = np.clip(k_rot * e_roll, -0.25, 0.25)
        ...
        if align_success:
            rospy.loginfo("✅ 前两轴姿态已完美调整至笔直向下！无任何坐标偏移。")
