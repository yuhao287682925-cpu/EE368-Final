...
class AutoContactDrawer:
    ...
    def joint_states_callback(self, msg):
        ...
        # 减去偏置得到真实的法向接触力并取绝对值，保证压力为正
        self.current_fz = abs(f_normal_raw - self.fz_bias) if self.calibrated else 0.0
        
        # 在空闲悬空且静止状态下进行温漂自动去皮（超低通偏置更新）
        if self.state == FREE_SPACE and self.is_static and self.current_fz < 2.0:
            self.fz_bias = 0.9995 * self.fz_bias + 0.0005 * f_normal_raw
            self.current_fz = abs(f_normal_raw - self.fz_bias)
    ...
    def execute_and_draw(self, csv_file):
        ...
        self.ny = start_wp['ny']
        self.nz = start_wp['nz']
        rospy.loginfo(f"🌐 提取到目标表面法向量: [{self.nx:.3f}, {self.ny:.3f}, {self.nz:.3f}]")
            
        # 1. 移动到首个航点法向上方 (对齐姿态)
        rospy.loginfo("🚗 正在沿法向移动到起点上方...")
        
        # 悬空点：在起点法线正方向 5cm 处
        hover_x = start_wp['x'] + 0.05 * self.nx
        hover_y = start_wp['y'] + 0.05 * self.ny
        hover_z = start_wp['z_nominal'] + 0.05 * self.nz
        
        quat = get_orientation_for_normal(self.nx, self.ny, self.nz)
        
        pose_goal = Pose()
        pose_goal.orientation = quat
        pose_goal.position.x = hover_x
        pose_goal.position.y = hover_y
        pose_goal.position.z = hover_z
        ...
        if not success:
            rospy.logerr("❌ 无法规划到起点上方！")
            return
            
        # 姿态对齐完成后，强制重新校准力矩传感器 (去皮)
        rospy.loginfo("🔄 正在姿态对齐后重新去皮校准力矩...")
        self.calibrated = False
        self.calibration_samples = []
        
        # 等待 1.5 秒零点去皮
        rospy.loginfo("⏳ 等待 1.5 秒进行零点去皮...")
        rospy.sleep(1.5)
        ...
        for wp in raw_waypoints:
            # 根据 3D 接触点和基准点的差值，平移整个轨迹
            dx_ref = contact_pose.position.x - u_ref_x
            dy_ref = contact_pose.position.y - u_ref_y
            dz_ref = contact_pose.position.z - u_ref_z
            
            aligned_wp = {
                'x': wp['x'] + dx_ref,
                'y': wp['y'] + dy_ref,
                'z_nominal': wp['z_nominal'] + dz_ref,
                'nx': wp['nx'],
                'ny': wp['ny'],
                'nz': wp['nz'],
                'phase': wp['phase'],
        ...
        for i, wp in enumerate(aligned_waypoints):
            ...
            # 位置伺服走点循环
            while not rospy.is_shutdown():
                # 1. 计算与目标点的距离 (基于所绘表面法向平面)
                err_vec = np.array([target_x - self.current_x, target_y - self.current_y, target_z - self.current_z])
                normal_vec = np.array([self.nx, self.ny, self.nz])
                err_normal = np.dot(err_vec, normal_vec) * normal_vec
                err_plane = err_vec - err_normal
                dist_to_target = np.linalg.norm(err_plane) # 仅看表面投影平面内的距离
                ...
                if wp['phase'] in ['draw', 'touch_down'] and dist_to_target > 0.01:
                    movement = math.hypot(math.hypot(self.current_x - prev_servo_x, self.current_y - prev_servo_y), self.current_z - prev_servo_z)
                    if movement < 0.0001: # 单周期全空间位移极小 (<0.1mm)
                        stuck_cnt += 1
                    else:
                ...
                # 5. 目标位置 = 理论位置 - 基准定深 + 单向泄压补偿 (沿法向)
                fixed_press_depth = 0.003
                if wp['phase'] in ['draw', 'touch_down']:
                    depth_offset = fixed_press_depth - z_offset_relief
                else:
                    depth_offset = 0.0
                    
                # 计算引入定深后的实际 3D 目标点
                target_x_cmd = target_x - depth_offset * self.nx
                target_y_cmd = target_y - depth_offset * self.ny
                target_z_cmd = target_z - depth_offset * self.nz
                
                dx = target_x_cmd - self.current_x
                dy = target_y_cmd - self.current_y
                dz = target_z_cmd - self.current_z
                ...
                cmd.twist.linear_z = np.clip(k_pos * dz, -0.06, 0.06)
        ...
        lift_cmd.twist.linear_x = 0.03 * self.nx
        lift_cmd.twist.linear_y = 0.03 * self.ny
        lift_cmd.twist.linear_z = 0.03 * self.nz  # 以 3cm/s 速度沿法线拔出画笔
